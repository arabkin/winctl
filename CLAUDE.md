...
| POST | `/api/cancel` | Cancel specific activities (JSON: restart_once, restart_schedule, lock_once, lock_schedule) |
| POST | `/api/reset` | Cancel everything and reset state |
...
- Login locks out per-IP after 3 failed attempts within a 5-minute sliding window; lockout lasts 15 minutes then auto-unlocks; successful login resets counter
